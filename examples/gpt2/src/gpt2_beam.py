...
parser.add_argument('--final_checkpoint', default=None, type=str, help='final PEFT checkpoint')
...
def _load_weight(state_dict):
    ...
    for key in state_dict_tmp:
        # If the key contains 'module.', remove the prefix (from DataParallel/DistributedDataParallel)
        new_key = key.replace('module.', '')
        new_state_dict[new_key] = state_dict_tmp[key]
...
if __name__ == '__main__':
    ...
    lm_net = TracGPT2LMModel(model, config)

    if args.final_checkpoint is not None:
        print('loading model PEFT weight.')
        cp = torch.load(args.final_checkpoint, map_location=torch.device('cpu'))
        state_dict = _load_weight(cp['model_state_dict'])
        lm_net.load_state_dict(state_dict, strict=False)

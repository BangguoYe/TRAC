...
from model import GPT2Config, GPT2LMModel, TracGPT2LMModel
from exp_utils import create_exp_dir

from trac import mark_lora_layernorm_cls_trainable, get_peft_model_state_dict, compute_trainable_param

def parse_list(value):
    # Assume the input is a comma-separated string, e.g., "q,k,v,proj,fc1,fc2"
    return value.split(',')
...
parser.add_argument('--target_modules', type=parse_list, default=['q', 'v'], 
                    help="A list of target modules (default: ['q', 'v'])")
...
def train_validate(
    ...
    for idx, data in enumerate(train_loader):
        ...
        if train_step % args.save_interval == 0: 
            if args.rank == 0:
                model_path = os.path.join(args.work_dir, f'checkpoint-{train_step}.pt')
                print('saving checkpoint', model_path)
                torch.save({'model_state_dict': get_peft_model_state_dict(model)}, model_path)
...
if __name__ == '__main__':
    ...
    if args.model_card == 'gpt2.md':
        config = GPT2Config(
            ...
            tensor_rank_A=tensor_rank_A,
            tensor_rank_B=tensor_rank_B,
            scale_shared_tt_cores=True,
            param_structure=None,
            tensor_init='TTNorm',
            scale_init='ones',
            vector_activation='none',
            use_fast_tt=False,
        )
    ...
    peft_model = TracGPT2LMModel(model, config)
    
    lm_net = peft_model.to(args.device)

    if args.lora_dim > 0:
        mark_lora_layernorm_cls_trainable(lm_net)
    
    total_param, lora_trainable_param = compute_trainable_param(lm_net)
    print("--- Model parameter statistics ---")
    print(f'***** Total param: {total_param}')
    print(f'***** Total lora trainable param: {lora_trainable_param}')
    print(f'***** Lora trainable ratio: {lora_trainable_param / total_param * 100:.4f} %')

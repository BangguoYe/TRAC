...
from trac import Linear, create_shared_params, TracLayer
...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False,\
                 r=1, lora_alpha=32, lora_dropout=0.0):
...
class MLP(nn.Module):
    def __init__(self, n_state, config, r=1, lora_alpha=32, lora_dropout=0.0):  # in MLP: n_state=3072 (4 * n_embd)
...
class TracGPT2LMModel(nn.Module):
    def __init__(self, gpt2_model, config, only_qv=True, lora_layer=None):
        super(TracGPT2LMModel, self).__init__()

        print(f'Trac target_modules: {config.target_modules}')
        self.only_qv = only_qv
        if lora_layer:
            self.lora_layer = lora_layer
        else:
            self.lora_layer = list(range(len(gpt2_model.transformer.h)))

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.shared_params = create_shared_params(peft_config=config,
                                            backbone_model=None,
                                            target_modules=config.target_modules[0],
                                            hidden_size=config.n_embd, 
                                            mlp_hidden_dim=4 * config.n_embd, 
                                            matrix_rank=config.lora_attn_dim)
        
        for k1, v1 in self.shared_params.items():
            for k2, v2 in v1.items():
                for k3, v3 in v2.items():
                    for k4, v4 in v3.items():
                        setattr(self, f'{k3}_param_{k1}_lora_{k2}_{k4}', v4)

        for t_layer_i, blk in enumerate(gpt2_model.transformer.h):
            # If we only want few lora layer instead of all
            if t_layer_i not in self.lora_layer:
                continue

            if self.only_qv == True:
                w_qkv_linear = blk.attn.c_attn
                self.dim = w_qkv_linear.in_features

                lora_q = TracLayer(peft_config=config, in_features=config.n_embd, out_features=config.n_embd, r=config.lora_attn_dim,\
                                        module_name='q', shared_params=self.shared_params, \
                                        lora_alpha=config.lora_attn_alpha, lora_dropout=config.lora_dropout)
                
                lora_v = TracLayer(peft_config=config, in_features=config.n_embd, out_features=config.n_embd, r=config.lora_attn_dim,\
                                        module_name='v', shared_params=self.shared_params, \
                                        lora_alpha=config.lora_attn_alpha, lora_dropout=config.lora_dropout)
                blk.attn.c_attn = LoRA_qkv(w_qkv_linear, lora_q, None, lora_v, config, only_qv=self.only_qv)
         
        self.peft_gpt2 = gpt2_model
    ...
    def load_weight(self, state_dict):
        ...
        for n, p in self.peft_gpt2.transformer.named_parameters():
            if n not in state_dict:
                state_dict[n] = p

        self.peft_gpt2.transformer.load_state_dict(state_dict, strict=False)
        self.peft_gpt2.set_tied()
    ...
    def forward(
        ...
        return self.peft_gpt2(input_ids, 
        lm_labels, 
        lm_mask, 
        past, 
        len_past, 

...
def add_optimizer_params(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--scale_factors_lr', type=float, default=0.0001, help='softmax part learning rate')  
...
def create_grouped_parameters(model, no_decay_bias, args):
    if not no_decay_bias:
        optimizer_grouped_parameters = [
            # Parameters named tt_dim_scale_factors or tt_rank_scale_factors use scale_factors_lr
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if (("tt_dim_scale_factors" in n) or ("tt_rank_scale_factors" in n))
                    and p.requires_grad
                ],
                "lr": args.scale_factors_lr,
            },
            # All other trainable parameters use the default learning rate
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if (("tt_dim_scale_factors" not in n) and ("tt_rank_scale_factors" not in n))
                    and p.requires_grad
                ],
                "lr": args.lr,
            }
        ...
        optimizer_grouped_parameters = [
            # tt_dim_scale_factors or tt_rank_scale_factors with weight decay enabled
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if (("tt_dim_scale_factors" in n) or ("tt_rank_scale_factors" in n))
            ...
            # tt_dim_scale_factors or tt_rank_scale_factors without weight decay
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if (("tt_dim_scale_factors" in n) or ("tt_rank_scale_factors" in n))
            ...
            # All other trainable parameters with weight decay enabled
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if (("tt_dim_scale_factors" not in n) and ("tt_rank_scale_factors" not in n))
            ...
            # All other trainable parameters without weight decay
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if (("tt_dim_scale_factors" not in n) and ("tt_rank_scale_factors" not in n))
...
def create_adam_optimizer_from_args(model, args, grouped_parameters=None):
    if grouped_parameters is None:
        grouped_parameters = create_grouped_parameters(model, args.no_decay_bias, args)
